...
@argument("name", str, prompt_method="input",
          prompt_kwargs={"prompt": "👤 What's your name? ", "placeholder": "Enter your full name"})
@argument("email", str, prompt_method="input", 
...
@argument("password", str, prompt_method="input",
          prompt_kwargs={"prompt": "🔐 Enter password: ", "password": True})
@argument("confirm_password", str, prompt_method="input",
...
def setup_password(password: str, confirm_password: str):
    ...
    strength_msg = GumWrapper.style(
        f"Password strength: {strength}",
        foreground=strength_color
    )
    print(strength_msg)
...
@argument("description", str, prompt_method="input",
          prompt_kwargs={"prompt": "📝 Description: ", "placeholder": "A brief description of your project"})
@argument("version", str, prompt_method="input", 
          prompt_kwargs={"prompt": "🏷️  Initial version: ", "value": "1.0.0"})
@argument("author", str, prompt_method="input",
          prompt_kwargs={"prompt": "👨‍💻 Author: "})
...
def initialize_project(project_name: str, description: str, version: str, author: str):
    ...
    metadata_display = GumWrapper.join(
        [GumWrapper.style(item, foreground="cyan") for item in metadata],
        vertical=True,
        align="left"
    )
    
    project_box = GumWrapper.style(
        metadata_display,
        border="double",
        padding="1 2"
    )
    ...
    steps = ["Creating directory structure", "Initializing git repository", "Creating config files"]
    for step in steps:
        print(f"   ✓ {step}")
    
    complete = GumWrapper.style(
