...
def demo_system_monitoring_table():
    ...
    headers = ["Server", "CPU %", "Memory %", "Disk %", "Network I/O", "Uptime", "Status"]
    data = [
    ...
    try:
        ...
        for server_data in data:
            ...
            status_color = "green" if "🟢" in status else "yellow" if "🟡" in status else "red"
            
            server_line = GumWrapper.style(
...
def demo_project_tracking_table():
    ...
    headers = ["Task ID", "Title", "Assignee", "Priority", "Status", "Due Date", "Progress"]
    data = [
        ["TSK-001", "User Authentication", "Alice", "High", "In Progress", "2024-01-20", "75%"],
        ["TSK-002", "Database Migration", "Bob", "Critical", "Blocked", "2024-01-18", "30%"],
        ["TSK-003", "UI Redesign", "Carol", "Medium", "Testing", "2024-01-25", "90%"],
        ["TSK-004", "API Documentation", "David", "Low", "Not Started", "2024-01-30", "0%"],
        ["TSK-005", "Performance Optimization", "Alice", "High", "Complete", "2024-01-15", "100%"],
        ["TSK-006", "Security Audit", "Eve", "Critical", "In Progress", "2024-01-22", "45%"],
        ["TSK-007", "Mobile Responsiveness", "Carol", "Medium", "In Progress", "2024-01-28", "60%"]
    ]
    ...
    try:
        ...
        for status, tasks in status_groups.items():
            status_styled = GumWrapper.style(f"\n{status}:", foreground="cyan", bold=True)
            print(status_styled)
            for task in tasks:
...
def demo_financial_summary_table():
    ...
    data = [
        ...
        ["Investments", "Portfolio", "$1,234,567.89", "-$15,430.50", "-1.2%", "Yesterday"],
    ...
    try:
        ...
        for account_data in data:
            ...
            account_line = GumWrapper.style(
                f"{account:>15}: {balance:>15} ({change:>10})",
                foreground=change_color
            )
            print(account_line)
        
        # Display totals
        total_line = GumWrapper.style(
