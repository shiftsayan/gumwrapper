...
# Valid examples
@argument("valid_choice", str, prompt_method="choose", choices=["A", "B", "C"], limit=1)
...
def demonstrate_invalid_examples():
    ...
    for i, example in enumerate(invalid_examples, 1):
        ...
        code_display = GumWrite.style(
            example["code"], foreground="red", border="dashed", padding="1"
        )
        print(code_display)
...
def show_valid_parameters():
    ...
    for method, params in valid_params.items():
        ...
        method_display = GumWrite.style(f"{method:>8}", foreground="cyan", bold=True)
        params_display = GumWrite.style(params_str, foreground="green")
    ...
    print(
        "   Output commands like 'style', 'format', 'join' are used directly via GumWrapper"
    )

    input_note = GumWrite.style(
