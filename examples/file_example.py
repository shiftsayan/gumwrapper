...
@argument("config_file", str, prompt_method="file",
...
def load_config_file(config_file: str):
    ...
    info_display = GumWrapper.join(file_info, vertical=True, align="left")
    file_box = GumWrapper.style(
        info_display,
        foreground="cyan",
        border="rounded",
        padding="1 2"
    ...
    if os.path.exists(config_file):
        ...
        warning = GumWrapper.style(
            "⚠️  Selected file does not exist!",
            foreground="red",
            bold=True
        )
        print(f"\n{warning}")
...
@argument("project_dir", str, prompt_method="file",
          prompt_kwargs={"path": ".", "file_selection": False, "directory_selection": True})
@argument("create_structure", bool, prompt_method="confirm",
...
def setup_project_directory(project_dir: str, create_structure: bool):
    ...
    info_display = GumWrapper.join(dir_info, vertical=True, align="left")
    dir_box = GumWrapper.style(
        info_display,
        foreground="blue",
        border="rounded",
        padding="1 2"
...
@argument("backup_source", str, prompt_method="file",
          prompt_kwargs={"path": ".", "file_selection": False, "directory_selection": True})
@argument("backup_dest", str, prompt_method="file", 
          prompt_kwargs={"path": ".", "file_selection": False, "directory_selection": True})
@argument("confirm_backup", bool, prompt_method="confirm",
...
def backup_directory(backup_source: str, backup_dest: str, confirm_backup: bool):
    ...
    info_display = GumWrapper.join(backup_info, vertical=True, align="left")
    backup_box = GumWrapper.style(
        info_display,
        foreground="purple",
        border="thick",
        padding="1 2"
...
@argument("log_file", str, prompt_method="file",
...
def view_log_file(log_file: str):
    ...
    details_display = GumWrapper.join(file_details, vertical=True, align="left")
    file_box = GumWrapper.style(
        details_display,
        foreground="green",
        border="rounded",
    ...
    if os.path.exists(log_file) and os.access(log_file, os.R_OK):
        ...
        options_display = GumWrapper.join(
            [f"• {option}" for option in view_options],
            vertical=True
        )
        
        options_box = GumWrapper.style(
            f"Available options:\n{options_display}",
            foreground="cyan",
            border="dashed",
            padding="1"
        ...
        error = GumWrapper.style(
            "❌ Cannot access selected log file",
            foreground="red",
            bold=True
        )
        print(f"\n{error}")
