...
def create_git_commit(commit_message: str, additional_notes: str):
    ...
    formatted_message = GumPrompt.style(
        commit_message, foreground="cyan", border="rounded", padding="1"
    )
    print(formatted_message)

    if additional_notes.strip():
        notes_formatted = GumPrompt.style(
            f"Additional Notes:\n{additional_notes}",
            foreground="yellow",
            border="dashed",
            padding="1",
    ...
    success = GumPrompt.style(
        "✅ Commit created successfully!", foreground="green", bold=True
    )
    print(f"\n{success}")
...
def create_bug_report(bug_description: str, environment_info: str):
    ...
    for title, content, color in sections:
        section = GumPrompt.style(
            f"{title}:\n{content}", foreground=color, border="rounded", padding="1"
        )
        print(f"\n{section}")
...
@argument(
    ...
    prompt_method="write",
    prompt_kwargs={
        "placeholder": "Write documentation for this code...\n\nThis function does:\n- \n- \n\nParameters:\n- \n\nReturns:\n- "
    },
)
...
def document_code(code_snippet: str, documentation: str):
    ...
    code_display = GumPrompt.style(
        f"Code:\n{code_snippet}", foreground="green", border="thick", padding="1"
    )
    print(code_display)

    # Display documentation
    doc_display = GumPrompt.style(
        f"Documentation:\n{documentation}",
        foreground="cyan",
        border="rounded",
        padding="1",
    ...
    combined = GumPrompt.join(
        [
            GumPrompt.style("CODE", foreground="green", bold=True),
            code_snippet,
            "",
            GumPrompt.style("DOCUMENTATION", foreground="cyan", bold=True),
            documentation,
        ],
        vertical=True,
        align="left",
    ...
    final_output = GumPrompt.style(combined, border="double", padding="1 2")
    ...
    for line in lines:
        if line.strip().startswith("- [ ]"):
            ...
            formatted_lines.append(
                GumPrompt.style(line, foreground="yellow", bold=True)
            )
        elif line.strip().startswith("Date:") or line.strip().startswith("Attendees:"):
            # Highlight metadata
            formatted_lines.append(GumPrompt.style(line, foreground="cyan"))
        else:
            formatted_lines.append(line)

    formatted_notes = "\n".join(formatted_lines)

    notes_display = GumPrompt.style(formatted_notes, border="rounded", padding="1 2")
