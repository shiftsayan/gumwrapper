...
@argument("delete_confirm", bool, prompt_method="confirm",
          prompt_kwargs={"message": "⚠️  Are you sure you want to delete all files?"})
@argument("backup_confirm", bool, prompt_method="confirm", 
...
def delete_files_with_confirmation(delete_confirm: bool, backup_confirm: bool):
    ...
    if backup_confirm:
        ...
        result = GumWrapper.style("Backup completed successfully!", 
                                foreground="green", bold=True)
        print(result)
    
    print("🗑️  Deleting files...")
    success_msg = GumWrapper.style(
...
@argument("agree_terms", bool, prompt_method="confirm",
          prompt_kwargs={"message": "Do you agree to the terms and conditions?"})
@argument("newsletter", bool, prompt_method="confirm",
          prompt_kwargs={"message": "Subscribe to our newsletter?"})
@argument("analytics", bool, prompt_method="confirm", 
          prompt_kwargs={"message": "Allow anonymous analytics data collection?"})
@command("setup_wizard", description="Application setup wizard with multiple confirmations")
def setup_wizard(agree_terms: bool, newsletter: bool, analytics: bool):
    ...
    if not agree_terms:
        error_msg = GumWrapper.style(
            "❌ Cannot proceed without agreeing to terms!",
            foreground="red",
            bold=True
        )
        print(error_msg)
...
@argument("proceed", bool, prompt_method="confirm",
...
def deploy_to_production(proceed: bool):
    ...
    if proceed:
        ...
        steps = ["Building application", "Running tests", "Uploading assets", "Updating servers"]
        for step in steps:
            print(f"   • {step}...")
