...
from gumwrapper import GumPrompt, GumWrite, argument
...
@argument(
    "age", int, prompt_method="input", prompt="How old are you? ", placeholder="25"
)
@argument(
    "favorite_color",
    str,
...
def create_profile(name: str, age: int, favorite_color: str, confirm_info: bool):
    ...
    profile_display = GumWrite.join(profile_info, vertical=True, align="left")
    profile_box = GumWrite.style(
        profile_display, foreground="cyan", border="rounded", padding="1 2", bold=True
    )
    print(profile_box)
...
def install_packages(packages: str):
    ...
    success = GumWrite.style("Installation completed!", foreground="green", bold=True)
    print(f"\n{success}")


@argument("config_file", str, prompt_method="file", path=".")
@argument(
    "load_backup", bool, prompt_method="confirm", message="Load backup configuration?"
)
...
def git_commit(commit_message: str):
    ...
    message_display = GumWrite.style(
        commit_message, foreground="green", border="thick", padding="1"
    )
